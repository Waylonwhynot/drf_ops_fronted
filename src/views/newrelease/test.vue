<template>
  ...
  <div class="release">
    ...
    <div class="add_app">
      <a-button @click="showaddModal" icon="plus">
        ...
        <a-modal v-model="addmodelvisible" title="新建应用" @ok="handleaddappOk">
          ...
          >
            <a-form-model-item ref="app_name" label="应用名称" prop="app_name">
              <a-input
                v-model="app_form.app_name"
                @blur="
          () => {
            $refs.app_name.onFieldBlur();
          }
        "
              />
            </a-form-model-item>
            <a-form-model-item ref="app_id" label="唯一标识符" prop="app_id">
              <a-input
                v-model="app_form.app_id"
                @blur="
          () => {
            $refs.app_id.onFieldBlur();
          }
        "
              />
            </a-form-model-item>
      ...
      <a-modal width="900px" v-model="visible1" title="新建常规发布 - 订单服务" @ok="handleOk" :footer="null">
        ...
        "
              ...
              <div class="steps-action">
                <a-button :disabled="next_status_control()" v-if="current < steps.length - 1"
...
export default {
  ...
  methods: {
    ...
    get_release_app_data(){
      this.$axios.get(`${this.$settings.host}/release/list/`,)
        .then((res)=>{
      ...
      this.$refs.addappruleForm.validate(valid => {
        if (valid) {
          let data = {
            ...
            desc:this.app_form.desc,
          }
          console.log('>>>>>',data);
          this.$axios.post(`${this.$settings.host}/release/list/`,data)
            .then((res)=>{
