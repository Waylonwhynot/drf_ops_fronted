<template>
  <div class="app-container">
    ...
    <el-table
      ...
      border
      style="width: 100%">
      <el-table-column label="告警ID" prop="fingerprint"></el-table-column>
      <el-table-column label="环境" prop="env" width="100"></el-table-column>
      <el-table-column label="等级" prop="eventLevel"  width="100"></el-table-column>
      <el-table-column label="告警业务" prop="eventService"  width="100"></el-table-column>
      <el-table-column label="告警主机" prop="hostname"></el-table-column>
      <el-table-column label="告警详情" prop="eventMessage"></el-table-column>
      <el-table-column label="状态" width="90">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status==='resolved'" type="primary" size="mini">已恢复</el-button>
          <el-button v-else type="warning" size="mini">故障中</el-button>
        </template>
      </el-table-column>
      <el-table-column label="告警分类" prop="alertType"></el-table-column>
      <el-table-column label="故障时长(s)" prop="alertTime"></el-table-column>
...
import {getPromAlerts} from "@/api/monitor/alertcenter";
...
export default {
  ...
  methods: {
    async fetchData() {
      const {data} = await api.getPromAlerts(this.form)
      this.tableData = data.results
      this.total = data.count
    },
    async fetchAlertData() {
      //  告警类型
      const data_alert_type = await api.getPromAlertType()
      this.alert_type_options = data_alert_type.data.results
      //  告警状态
      const data_alert_status = await api.getPromAlertStatus()
      this.alert_status_options = data_alert_status.data.results
      //  环境
      const data_alert_envs = await api.getPromAlertEnvs()
